<template>
  <div class="main">
    <div class="content">
      <div class="swiper-bottom">
        <p>
          <span>★</span>
          <span>30天无忧退货</span>
        </p>
        <p>
          <span>★</span>
          <span>48小时快速退款</span>
        </p>
        <p>
          <span>★</span>
          <span>满88元免邮费</span>
        </p>
      </div>
      <div v-show="!isShow" class="cartGoodsListWrap">
        <div class="cartGoodsItem" v-for="item in shopcarList" :key="item.id">
          <div class="isCheckItem">
            <img
              @click="handChecked(item.checked,item.product_id)"
              v-if="item.checked===1"
              src="data:image/png;base64,"
              alt
            />
            <img
              @click="handChecked(item.checked,item.product_id)"
              v-if="item.checked===0"
              src="data:image/png;base64,"
              alt
            />
          </div>
          <div class="goodsImg">
            <img v-lazy="item.list_pic_url" alt />
          </div>
          <div class="cartGoodsMsg">
            <p>{{item.goods_name}}</p>
            <p>{{item.goods_specifition_name_value}}</p>
            <p>￥{{item.retail_price}}</p>
          </div>
          <div class="cartGoodsNum">
            <span>X {{item.number}}</span>
          </div>
        </div>
      </div>
      <div v-show="isShow" class="cartGoodsListWrap">
        <div class="cartGoodsItem" v-for="item in shopcarList" :key="item.id">
          <div class="isCheckItem">
            <img
              v-if="item.checked===1"
              src="data:image/png;base64,"
              alt
            />
            <img
              v-if="item.checked===0"
              src="data:image/png;base64,"
              alt
            />
          </div>
          <div class="goodsImg">
            <img v-lazy="item.list_pic_url" alt />
          </div>
          <div class="cartGoodsMsg">
            <p>已选择:</p>
            <p></p>
            <p style="color:red">￥{{item.retail_price}}</p>
          </div>
          <div class="cartGoodsNum">
            <div>-</div>
            <div>{{item.number}}</div>
            <div>+</div>
          </div>
        </div>
      </div>
    </div>
    <div v-show="!isShow" class="shopcarFooter">
      <div class="isCheckItem">
        <img
          src="data:image/png;base64,"
          alt
        />
        <img
          src="data:image/png;base64,"
          alt
        />
      </div>
      <div class="cartMsgAll">
        <span>已选({{cartTotal.goodsCount}})</span>
        <span>￥{{cartTotal.goodsAmount}}</span>
      </div>
      <div class="cartAllDoButton" @click="()=>{this.isShow=true}">编辑</div>
      <div class="cartAllDoButton pay">下单</div>
    </div>
    <div v-show="isShow" class="shopcarFooter">
      <div class="isCheckItem">
        <img
          src="data:image/png;base64,"
          alt
        />
        <img
          src="data:image/png;base64,"
          alt
        />
      </div>
      <div class="cartMsgAll">
        <span>已选({{cartTotal.checkedGoodsCount}})</span>
      </div>
      <div class="cartAllDoButton" @click="()=>{this.isShow=false}">完成</div>
      <div class="cartAllDoButton pay">删除所选</div>
    </div>
  </div>
</template>

<script>
import { mapState } from "vuex";
export default {
  data() {
    return {
      isShow: false
    };
  },
  computed: mapState({
    shopcarList: store => store.shopcar.shopcarList,
    cartTotal: store => store.shopcar.cartTotal
  }),
  mounted() {
    this.$store.dispatch("shopcar/_getShopcarList");
  },
  methods: {
    handChecked(checked, procuctId) {
      if (checked === 0) {
        this.$store.dispatch("shopcar/_setChecked", {
          isChecked:1,
          productIds: procuctId
        });
      }else{
         this.$store.dispatch("shopcar/_setChecked", {
          isChecked:0,
          productIds: procuctId
        });
      }
    }
  }
};
</script>

<style lang="scss">
@import "index.scss";
</style>